...
if ( !empty($datosIng) )  {
    ...
    if ($AbmUsRol->alta($datosIng)){
        $mensaje = "<div class='alert alert-info' role='alert'>
        <i class='fas fa-check-circle mx-2'></i>Se asignó correctamente el rol del usuario.
        </div>";
    } else {
        $mensaje = "<div class='alert alert-danger' role='alert'>
        <i class='fas fa-times-circle mx-2'></i>Hubo un error al asignar el rol del usuario.
        </div>";
    }
} else {
...
<div class="container p-2" id=ejercicio> <!-- Comienzo div resultado -->
	<h4 class="text-center mb-4"><i class="fas fa-user-tag mx-2"></i>Resultado:</h4>
    <?=$mensaje?>
